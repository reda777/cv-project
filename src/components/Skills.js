...
class Skills extends Component {
  ...
  renderSkillName(id) {
    ...
    let skillNameDiv =
      renderPart === "skillName" && renderId === id ? (
        <input
          type="text"
          id="skillName"
          value={this.state.rows[id].skillName}
          onChange={this.handleChange}
          onKeyDown={this.handleEnter}
        />
      ) : (
        <div className="skillName" onClick={this.handleClick}>
          {this.state.rows[id].skillName}
        </div>
      );
    return skillNameDiv;
  }
  renderSkillRange(id) {
    ...
    const renderId = this.props.renderId;
    let skillRangeDiv =
      renderPart === "skillRange" && renderId === id ? (
        <input
          type="range"
          min="1"
          max="20"
          name="skillRange"
          id="skillRange"
          value={this.state.rows[id].skillRange}
          onChange={this.handleChange}
          onKeyDown={this.handleEnter}
        />
      ) : (
        <div data-id={id} className="skillRange" onClick={this.handleClick}>
          <div
            className="skillRange insideSkillRange"
            style={{ width: (70 / 20) * this.state.rows[id].skillRange + "px" }}
          ></div>
        </div>
      );
